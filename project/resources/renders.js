...
function render() {
  ...
  camera = [
    px + settings.D * Math.sin(degToRad(facing)),
    py + 20,
    pz + settings.D * Math.cos(degToRad(facing)),
  ...
  if (cameraLibera) {
    camera = [
      settings.D * 7 * Math.sin(settings.PHI) * Math.cos(settings.THETA),
...
function drawPizza(programInfo) {
  if (!endGame) {
    ...
    if (!leavePizza[deliveredPizzas - 1]) {
      //la pizza è dentro l'auto
      matrix_pizza = m4.copy(matrix_chassis);
      objToDraw.uniforms.u_world = matrix_pizza;
    } else if (leavePizza[deliveredPizzas - 1]) {
      //la pizza viene rilasciata nell'area di consegna
      matrix_pizza = m4.identity();
      matrix_pizza = m4.translate(
        matrix_pizza,
        pxPizza,
        pyPizza + 0.5,
        pzPizza
      );
      matrix_pizza = m4.scale(matrix_pizza, 2, 0.3, 2);
      objToDraw.uniforms.u_world = matrix_pizza;
    }
...
function drawAdvert() {
  ...
  matrix = m4.scale(matrix, 0.3, 0.03, 0.3);
  objToDraw.uniforms.u_world = matrix;
...
function drawWheels(programInfo) {
  ...
  matrix_wheels = m4.scale(matrix_wheels, 0.6, 0.6, 0.6);
