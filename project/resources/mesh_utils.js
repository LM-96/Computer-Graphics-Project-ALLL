...
function find_in_face(face, ind_face, ind_vert) {
  ...
  for (k = 0; k < face[ind_face].n_v_e; k++)
    if (face[ind_face].vert[k] == ind_vert) break;
  return k;
}
...
function LoadSubdivMesh(mesh) {
  ...
  for (k = 1; k <= nface; k++) {
    for (ii = 0; ii < mesh.face[k].n_v_e; ii++) {
      ...
      if (
        buf[kk].vedge[count_fe[kk]] ==
        mesh.face[k].vert[(ii + 1) % mesh.face[k].n_v_e]
      ) {
    ...
    if (mesh.vert[i].bound)
      for (k = 0; k < mesh.vert[i].val - 1; k++) {
        ...
        for (l = 1; l < mesh.face[jj].n_v_e - il; l++) {
          mesh.vert[i].neigh[ic] =
            mesh.face[jj].vert[(l + ii) % mesh.face[jj].n_v_e];
          ic++;
        }
        il = 1;
      }
  ...
  for (i = 1; i <= nface; i++) {
    ...
    if (mesh.face[i].bound) {
      ...
      ijc = 1;
      while (
        mesh.vert[mesh.face[i].vert[(ij + 1) % mesh.face[i].n_v_e]].bound &&
        ijc <= mesh.face[i].n_v_e
      ) {
  ...
  delle facce intorno*/
    for (j = 0; j < mesh.face[i].n_v_e; j++) {
      ...
      for (l = 1; l < nval; l++) {
        if (ic != 0) {
          if (
            mesh.vert[jj].face[(l + ii) % nval] != mesh.face[i].neigh[ic - 1]
          ) {
