...
export interface SingleSignalSubscriber<S, D, R> {
    ...
    /**
     * Accept a subscription following the requested option, if possible, returning a **receipt**.
     ...
     * @return {SubscriptionNotAcceptedException} if the subscription can not be accepted
     */
    subscribe(options: SubscriptionOptions<S, D, R>): SubscriptionReceipt<S, D, R>

    /**
     * Remove a previously registered subscription, returning a new updated receipt.
...
export interface MultipleSignalSubscriber {
    ...
    unsubscribe<S, D, R>(subscriptionReceipt: SubscriptionReceipt<S, D, R>): void
}