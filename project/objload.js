...
function Buffers(positionBomber, normalBomber, texcoordBomber) {
  ...
  this.areCreated = function () {
    return (
      this.positions != null && this.normals != null && this.texcoords != null
    );
  };
...
function MeshObj(name, path) {
  ...
  this.createGlPNTBuffers = function (gl) {
    ...
    log(
      "createGlPNTBuffers() | position, normal and texcoord buffer created for object '" +
        this.name +
        "'"
    );
  };
  ...
  this.writeBufferFor = function (gl, varName, unbind = false) {
    ...
    gl.bufferData(
      gl.ARRAY_BUFFER,
      new Float32Array(this.getDataOf(varName)),
      gl.STATIC_DRAW
    );
    if (unbind) {
  ...
  this.writeToVertexShaderLocation = function (
    gl,
    shaders,
    varName,
    componentsPerIteration = 3,
    dataType = gl.FLOAT,
    normalize = false,
    stride = 0,
    offset = 0
  ) {
    var location = getShaderLocationFromName(
      shaders.locations.attributes,
      varName
    ).location;
...
class MeshObjLoader {
  ...
  add(name, path) {
    if (this.has(name)) {
      throw new Error(
        "An object with name " + name + " is already present. Unable to add"
      );
    }
...
class MeshObjDrawer {
  ...
  setProjection(fov, aspect, zmin, zmax) {
    ...
    this.gl.uniformMatrix4fv(
      this.shaderProgram.locations.projectionMatrix,
      false,
  ...
  setView(cameraPosition, target, up) {
    ...
    this.gl.uniformMatrix4fv(
      this.shaderProgram.locations.viewMatrix,
      false,
      this.matrices.view
    );
    this.gl.uniform3fv(
      this.shaderProgram.locations.viewWorldPosition,
      cameraPosition
    );
  ...
  setLightPosition(x, y, z) {
    this.gl.uniform3fv(
      this.shaderProgram.locations.lightWorldDirection,
      m4.normalize([x, y, z])
    );
  }
  ...
  drawObject(
    ...
    this.gl.uniformMatrix4fv(
      this.shaderProgram.locations.worldMatrix,
      false,
      objMoMatrix
    );
