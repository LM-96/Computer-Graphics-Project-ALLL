...
function FacetNormals(mesh) {
    ...
    if (mesh == null || mesh == undefined)
        return;
    if (mesh.vert == null || mesh.vert == undefined)
        return;
    if (mesh.face == null || mesh.face == undefined)
        return;
...
function glmReadOBJ(data, mesh) {
    ...
    for (j = 0; j < lines.length; j++) {
        ...
        switch (buf[0]) {
            ...
            case 'vt': // v, vn, vt 
                ...
                break;
            case 'mtllib':
                if (buf.length > 1 && buf[1].endsWith('.mtl'))
                    mtllibname = buf[1];
                break;
            case 'usemtl':
            ...
            case 'f': // face 
                ...
                if (f1[1])
                    tindex.push(parseFloat(f1[1])); // - 1;	//indice texcoord
                if (f1[2])
                    nindex.push(parseFloat(f1[2])); // - 1;	//indice normale
                vindex.push(parseFloat(f2[0])); // - 1;
                if (f2[1])
                    tindex.push(parseFloat(f2[1])); // - 1;
                if (f2[2])
                    nindex.push(parseFloat(f2[2])); // - 1;
                vindex.push(parseFloat(f3[0])); // - 1;
                if (f3[1])
                    tindex.push(parseFloat(f3[1])); // - 1;
                if (f3[2])
                    nindex.push(parseFloat(f3[2])); // - 1;
                ...
                if (f4 != null) {
                    ...
                    if (f4[1])
                        tindex[2] = parseFloat(f4[1]); // - 1;
                    if (f4[2])
                        nindex[2] = parseFloat(f4[2]); // - 1;
                    AddFace(vindex, tindex, nindex);
                }
                break;
        }
    ...
    FacetNormals(mesh);
    if (mesh.normal.length == 1)
        VertexNormals(mesh, 180); //il file non conteneva le normali ai vertici => vanno calcolate
    ...
    function AddFace(vindex, tindex, nindex) {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            if (tindex.length >= i + 1)
                face.textCoordsIndex[i] = tindex[i];
            if (nindex.length >= i + 1)
                face.normalVertexIndex[i] = nindex[i];
        }
        mesh.face.push(face);
        ...
        switch (buf[0]) {
            ...
            case 'newmtl':
                ...
                for (i = 0; i < mesh.materials.length && !material; i++) {
                    if (mesh.materials[i].name == buf[1])
...
//# sourceMappingURL=glm_utils.js.map